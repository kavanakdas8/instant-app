...
export const MobileContainer: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex justify-center items-stretch font-sans antialiased selection:bg-accent-pink/30">
      {/* On desktop, show full-width layouts; on mobile, show the mock phone wrapper */}
      <div className="w-full md:max-w-none max-w-md min-h-screen flex flex-col bg-black md:bg-black border-x md:border-none border-zinc-900 relative shadow-2xl overflow-hidden">
        {/* Top Notch/StatusBar Simulation for mobile aesthetics - hidden on desktop */}
        <div className="md:hidden h-6 bg-black border-b border-zinc-950 flex justify-between items-center px-6 text-[10px] text-zinc-500 select-none z-50">
          <span>9:41</span>
        ...
        {/* Core Content Area */}
        <main className="flex-1 flex flex-col overflow-y-auto relative no-scrollbar md:pb-0 pb-16">
          {children}
        </main>
      </div>
    </div>
